...
time = linspace(0,T,M);   
dt = time(2)-time(1);
H = 0.9; %Parameter needed to define the initial condition
...
if CriticalL == 0
    disp('Error: The test needs to be done with BCs wich are compatible with steady state solutions');
        ...
        for t = time(2:end)
            ...
            b1(1) = m*b; %To impose the boundary conditions
            b1(end) = m*b;
...
s = 1; %So for stability we set dt = 2*s*dx^2/(4+a*dx^2)
...
runtimeEE = zeros(length(tolerance),1); %Here I will store the execution time to reach a desired tolerance for the explicit scheme
numberEE = zeros(length(tolerance),1); %Here I will store the number of iterations required to reach those tolerances
count = 1;

for tol = tolerance
    tic; %Here I start taking the time
    u = u0;
    err = 1;
    j=1;
    while err>tol
    ...
    runtimeEE(count) = toc; %Here I stop taking the time and store that value as an entry of the vector
...
c=10^15; %Parameter for the penalization method

B = (speye(N)+CN); %Matrix for the Pseudo CN method
...
for tol = tolerance
    ...
    numberCN(count) = i-1;
    runtimeCN(count) = toc; %Stop taking time and storing it
    count = count + 1;
end
