...
%line to get the plot in the report, and uncomment lines 105-111 below.
...
for N = xrange
    ...
    %Important: since I have both dirichlet BCs I omit the first and last rows
    %and columns, since their coefficients are already fixed.

    %Assembling the stiffness matrix:
    d1 = [1/h(1);1./h(1:end-1) + 1./h(2:end);1/h(end)];
    d2 = - 1./h(1:end);
    A = diag(d1,0) + diag(d2,1) + diag(d2,-1); %Important: first and last rows/columns will be then
    %adapted accordingly to the boundary conditions. Btw if both the space
    %boundary conditions are homogeneus Dirichlet, these 2 rows and columns can
